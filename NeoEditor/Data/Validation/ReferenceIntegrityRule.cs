...
public class ReferenceIntegrityRule : IValidationRule
{
    public void Validate(IReadOnlyList<IEntity> entities, ValidationReport report)
    {
        ...
        foreach (var entity in entities)
        {
            ...
            foreach (var prop in refProps)
            {
                ...
                if (!GenericDataGridHelper.ReferenceLookups.ContainsKey(targetType))
                    continue;
        ...
        Type targetType, string? pattern, string? targetKey, ValidationReport report)
    {
        ...
        {
            var found = GenericDataGridHelper.ResolveEntityIdByTargetKey(targetType, rawId, targetKey);
        ...
        var resolved = GenericDataGridHelper.ResolveEntityIdByTargetKey(targetType, rawId, targetKey);
        if (resolved is null)
        {
            report.Warning(entity.Subject, colName,
