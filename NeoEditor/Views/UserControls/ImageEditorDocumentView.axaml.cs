...
using NeoEditor.Views.UserControls.ImageEditor;
using Ursa.Controls;
...
public partial class ImageEditorDocumentView : UserControl
{
    ...
    private ImageEditorDocument? _viewModel;
    ...
    public ImageEditorDocumentView()
    {
        ...
        OriginalImageViewer.PropertyChanged += OnImageViewerPropertyChanged;
        SelectionOverlayCanvas.PropertyChanged += OnSelectionOverlayCanvasPropertyChanged;
        UpdateViewModel(DataContext as ImageEditorDocument);
    ...
    private void UpdateViewModel(ImageEditorDocument? newViewModel)
    {
    ...
    {
        if (e.PropertyName is nameof(ImageEditorDocument.SelectedImage) or nameof(ImageEditorDocument.CropRect))
        {
            _interactiveSelection = null;
            UpdateSelectionOverlay();
        }
    ...
    private void OnImageViewerPropertyChanged(object? sender, AvaloniaPropertyChangedEventArgs e)
    {
        if (e.Property == BoundsProperty
            || e.Property == ImageViewer.ScaleProperty
            || e.Property == ImageViewer.TranslateXProperty
            || e.Property == ImageViewer.TranslateYProperty
            || e.Property == ImageViewer.SourceProperty
            || e.Property == ImageViewer.StretchProperty)
        {
    ...
    {
        var sourceWidth = _viewModel?.SelectedImage?.PixelSize.Width ?? 0;
        var sourceHeight = _viewModel?.SelectedImage?.PixelSize.Height ?? 0;
        return ImageSelectionViewportMapper.TryCreateGeometry(
            SelectionOverlayCanvas.Bounds.Size,
            sourceWidth,
            sourceHeight,
            OriginalImageViewer.Scale,
            OriginalImageViewer.TranslateX,
            OriginalImageViewer.TranslateY);
    }
    ...
    private bool TryGetDisplayedSelection(out ImageCropSelection selection)
    {
        ...
        if (_viewModel?.SelectedImage is { } image)
        {
    ...
    private void PrepareCropForProcessing()
    {
        if (_selectionInteraction.IsActive)
        {
            _selectionInteraction.End();
    ...
    private async void OnPixelateClick(object? sender, RoutedEventArgs e)
    {
        e.Handled = true;
