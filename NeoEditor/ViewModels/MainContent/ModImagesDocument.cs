...
public partial class ModImagesDocument : DocumentViewBase
{
    ...
    private readonly PhpParser _phpParser = App.ServiceProvider.GetRequiredService<PhpParser>();
    ...
    public ModImagePairDropHandler ImagePairDropHandler { get; } =
        App.ServiceProvider.GetRequiredService<ModImagePairDropHandler>();
    ...
    private async Task ImportImages()
    {
        ...
        {
            App.Notification.ShowSuccess(Loc["ImportImagesSuccessMessage", importedItems.Count, imageDirectory],
    ...
    private async Task RenameImagePair(ModImagePairItem? item)
    {
        ...
        {
            App.Notification.ShowWarning(Loc["RenameImagePairMissingFilesMessage"], Loc["RenameImagePairMissingFiles"]);
            return;
        }
        ...
        var dialog = App.ServiceProvider.GetRequiredService<RenameImagePairDialog>();
        ...
        try
        {
            var proposedNames = dialog.ViewModel.GetProposedNames();
            if (!TryRenameImagePairFiles(targetPair, proposedNames.NormalFileName, proposedNames.X2FileName,
                    out var renamedPair))
            {
                App.Notification.ShowWarning(Loc["RenameImagePairFileExists"], Loc["RenameImagePairFailed"]);
                return;
            }
            ...
            if (await SaveCoreAsync(notifySuccess: false))
            {
                ...
                App.Notification.ShowSuccess(Loc["RenameImagePairSuccessMessage", renamedPair.NormalImage],
                    Loc["RenameImagePairSuccess"]);
            }
        }
        catch (Exception ex)
        {
            App.Notification.ShowError(Loc["RenameImagePairFailedMessage", ex.Message], Loc["RenameImagePairFailed"]);
        }
    }
    ...
    private async Task<bool> SaveCoreAsync(bool notifySuccess)
    {
        ...
        try
        {
            ...
            var phpContent = _phpParser.GenerateImagePhp(imagePairs);
            ...
            if (notifySuccess)
            {
                App.Notification.ShowSuccess(Loc["SaveModImagesSuccessMessage", getImagesPath], Loc["SaveModImagesSuccess"]);
            }
        ...
        {
            App.Notification.ShowError(Loc["SaveModImagesFailedMessage", ex.Message], Loc["SaveModImagesFailed"]);
            return false;
        }
    }
    ...
    private void NotifyImportSkipSummary(IReadOnlyList<string> skippedFiles)
    {
        ...
        App.Notification.ShowWarning(Loc["ImportImagesSkippedMessage", preview], Loc["ImportImagesSkipped"]);
    }
