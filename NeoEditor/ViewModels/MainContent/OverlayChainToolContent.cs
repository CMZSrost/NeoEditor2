...
public partial class OverlayChainToolContent : LocalizedObservableObject
{
    ...
    public void LoadChain(string entityId, string subject, string entityType)
    {
        ...
        if (!GenericDataGridHelper.OverlayChainDisplay.TryGetValue(entityId, out var entries) || entries.Count == 0)
        {
            RefreshComputed();
            return;
        }
