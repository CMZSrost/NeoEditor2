...
public partial class ReferenceInspectorView : UserControl
{
    ...
    private async void OnOpenClick(object? sender, RoutedEventArgs e)
    {
        ...
        ViewServices.Get<NeoEditor.Plugins.DataViewer.Services.DataTableService>()?.NavigateToByEntityId(entityType, _vm.TargetEntityId);
        ...
        if (_vm.TargetModId > 0)
        {
            try
            {
                await using var edb = await ViewServices.EditorDbFactory
                    .CreateDbContextAsync();
                var modInfo = await edb.ModInfos.FirstOrDefaultAsync(m => m.ModId == _vm.TargetModId);
