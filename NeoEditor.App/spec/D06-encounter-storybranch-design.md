...
1. **一个节点 = 一个组件，只保留三个核心要素**：**图片（记忆）、标题（目标）、概率（可能性）**。
   节点卡 = 标题（第一行）+ `strImg` 图片（第二行，占主体 ~70%）+ 概率胶囊（第三行右侧），chip 在第三行左&中。
...
| 3. 每个节点应做成图片+title 的单组件，信息组件内布局 | 分支节点 = 4-5 个**竖排**元素：物品徽章（685-695）→ 概率胶囊（698-709）→ 条件徽章行（712-745）→ 目标徽章（747-771），无图片、无标题层级 | 节点信息以「徽章堆叠」而非「组件内布局」呈现，视觉散乱 | 节点卡 = 标题 + 图片主体 + chip/概率行（§四，v1.2） |
...
**v1.2 规格（用户 2026-08-08 二次反馈）：卡片三要素 + 主体化布局——标题第一行、图片第二行（占主体 ~70%）、第三行 chip 左&中 + 概率右侧**：

```
┌────────────────────────────────────┐  分支卡规格（v1.2）
│        便利店抢劫（居中标题）         │  ← 行1：标题 12px SemiBold #333，居中
│   ┌─────────────────────┐          │  ← 行2：图片 = 主体（168px ≈ 卡片宽 70%，
│   │      （图片）         │          │     高 110，居中，点击放大；无图 BookOpen 兜底）
│   └─────────────────────┘          │
│  [ID 237] [剧情]      68%(1.4)     │  ← 行3：左&中 = ID/类型 chip，右 = 概率胶囊
│   ↑ hover → tooltip 信息卡          │
└────────────────────────────────────┘
```
...
| 部件 | 规格 | 说明 |
|------|------|------|
| 整体 | Border，CornerRadius 6，Padding 10，Width 220-260（实现 240），MinHeight 与图片区匹配 | 当前卡：bg `#E3F2FD` / 边框 `#1565C0` 2px + 顶部 8px 标签「📍 当前剧情」；分支卡：bg `#FAFAFA` / 边框 `#E0E0E0` 1px；不满足前置条件的分支卡：`Opacity 0.5`（保留现状语义） |
| 行1：标题 | `e.Subject ?? $"Enc #{e.Id}"`，12px SemiBold #333，TextWrapping.Wrap，**独占第一行，居中**（不再与图片同行） | 分支卡整卡 `_refNode.WireNavigation`（Ctrl+Click 跳转 / Ctrl+RMB peek） |
| 行2：图片（主体） | **168×110**（≈ 卡片宽 70%），CornerRadius 6，`ClipToBounds`，bg `#0A000000`，**居中**；`_vis.LoadImage(e.Image)`（与 Hero 同源加载，内部 `StripNs` 处理 `0:` 命名空间前缀），`Stretch.Uniform` | 点击 → `_vis.OpenZoomableImage(bmp, e.Subject ?? e.Name)`（与 Hero 一致）；**无图 → `SymbolIcon`（`Symbol.BookOpen`，24px，`#999`）居中兜底，不崩溃**；当前卡与分支卡一致 |
| 行3：chip 行（左&中 chip \| 右概率） | Grid 双列 `[1* | auto]`：左侧 = `ID: {id}` chip（`#E3F2FD`/`#1565C0`）+ 类型 chip（映射见 4.2，9px）；右侧 = 概率胶囊 | v1.2：chip 与概率**同一行**，左&中放 chip，概率靠右；当前卡无概率 → 仅 chip 行 |
| 概率胶囊 | 文本 `{Weight:F1}({EffectiveProb:P2})`，≥50% 绿/≥10% 橙/其余灰，白字 9px Bold（见 4.4） | 三要素之一「可能性」；仅分支卡 |
| **tooltip 信息卡** | 分支卡整卡挂 `ToolTip`，内容为分支专用信息卡，**顺序：标题 → 物品（第二行）→ 描述 → 前置条件及满足情况 → 概率** | v1.2：**物品行用 `Item.Name`（非 Description）** 且置于第二行（标题之后）；描述（strDesc 截断 ~200 字）；条件每条 ¬ 前缀 + 名称 + ✓满足/✗不满足 着色（随过滤刷新）；**不占用卡片布局** |

**高度目标**：折叠态 = 标题 ~18px + 图片 110px + chip 行 ~20px + 内边距，≤ 170px；
「卡片 = 标题 + 图片（主体）+ chip/概率行」是硬约束——描述/条件/物品等复杂信息一律进 tooltip，
禁止追加常显徽章行。
...
| 节点 = 单组件卡（标题 → 图片主体 70% → chip/概率行），**描述/条件/物品进 tooltip 信息卡** | 用户反馈 3「每个节点应该做成单组件，图片+title…组件内布局紧凑展示」+ 2026-08-08 追加「复杂信息通过 tooltip 信息卡做；节点关注图片（记忆）、标题（目标）、概率（可能性）」+ 二次追加「图片占主体/标题第一行/chip 左中概率右；tooltip 物品用 Name 放第二行」 |
