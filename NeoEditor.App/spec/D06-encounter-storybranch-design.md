...
> 上承：D05 Creature 可视化设计（模板与决策体系沿用）+ 用户对「剧情分支」Tab 的三条反馈（2026-08-08）
...
1. **一个节点 = 一个组件**。每个 Encounter 节点是一张卡片：`strImg` 缩略图 + 标题，
   物品 / 概率 / 前置条件全部收进卡片内部，用组件内布局（Grid 双列 / 单行 StackPanel）紧凑呈现，
   而不是散成一串竖排徽章（用户反馈 3 的直接要求）。
...
仅 `opts`（isCurrent / 物品+概率 / 前置条件数 / 是否可导航）不同。

```
┌────────────────────────────────────┐  分支卡规格（折叠态）
│ ┌─────┐ 便利店抢劫                  │  ← 标题 12px SemiBold #333，可换行（与图片同行）
│ │ 52  │ ─────────────────────────  │  ← 行1（信息行 1/2）：ID chip + 类型 chip [+ 📋×n]
│ │ px  │ ID 237 · 剧情   📋×3        │     均为 9-10px chip；📋×n 可点击（见 §五）
│ │ 图  │ ─────────────────────────  │  ← 行2（信息行 2/2）：Grid 双列 [1* | auto]
│ │ 片  │ 🛡 撬棍 ×1      │ 68%(1.4)  │     左=物品触发徽章（无物品时隐藏，概率占整行）
│ └─────┘                            │     右=概率胶囊（权重+有效概率）
└────────────────────────────────────┘
```
...
| 标题（右侧文本区） | `e.Subject ?? $"Enc #{e.Id}"`，12px SemiBold #333，TextWrapping.Wrap，与图片同行（图片行即标题行） | 分支卡整卡 `_refNode.WireNavigation`（Ctrl+Click 跳转 / Ctrl+RMB peek），resolved 时挂 `BuildRefTooltip`（Type/Price/LootChance） |
| 行1：ID + 类型 | `ID: {id}` chip（`#E3F2FD`/`#1565C0`）+ 类型 chip | 类型映射见 4.2；分支卡追加「📋×{n}」chip（n = 目标前置条件数，仅 n>0 时显示） |
| 行2：物品 \| 概率 | Grid 双列 `[1* | auto]` | 见 4.3 / 4.4 |
| 行3（展开态） | 前置条件徽章 WrapPanel | 仅点击 📋×n 后显示，复用现有条件徽章渲染（712-745 行逻辑：¬ 前缀、红/绿语义色、过滤未选中灰化） |

**高度目标**：折叠态 = 图片 52px + 两行信息（2×~18px）+ 内边距，≤ 96px；
「信息行 ≤ 2 行」是硬约束——任何新信息必须收进行 1/行 2 的组件内布局，禁止追加竖排徽章行。
...
- 有物品（`resp.Item is not null`）：`🛡 {Item.Description}{×mult}` 徽章，
  复用 `_refNode.BadgeForEntity(enc, resp.Item, …)`（导航 + tooltip，与 BuildResponsesPanel
  111 行同款），配色 `#E3F2FD`/`#1565C0`；数量 `×{n}` 仅 n>1 时显示。
- 物品未解析（`resp.ItemId is not null` 但查找失败）：灰色 `Item #{id}` 徽章（现状语义保留）。
- 无物品（`=开头` 的空物品选项）：**整行不显示**，概率胶囊占满行 2（去噪）。
...
- 分支卡渲染与 `BuildMermaidText` 都只消费 `List<BranchData>`，二者结构性不可能漂移。

## 五、交互设计

| 交互 | 行为 | 现状 → 重构 |
|------|------|-------------|
| 跳转 | 分支卡（resolved）Ctrl+Click → 目标 Encounter，Ctrl+RMB → peek；hover → `BuildRefTooltip` | 保留（WireNavigation 从目标徽章移挂到整卡） |
| 图片放大 | 缩略图点击 → `OpenZoomableImage`（标题 = 节点名） | 新增（Hero 同款，79-83 行参考） |
| 前置条件过滤 | 复选框区（分支目标前置条件的并集，☑/¬ 样式不变）：勾选 → 重算 ValidTotalWeight → 不满足分支卡 Opacity 0.5 + 概率 0%，其余分支概率按新总和重算；**同步刷新卡片与 Mermaid** | 保留（662-674 行逻辑原样平移进纯函数） |
| 前置条件展开 | 分支卡「📋×{n}」chip 点击 → 展开/收起该卡条件徽章行（默认收起）；展开态徽章复用现有渲染（¬ 前缀、语义色、未选中灰化） | 从「常显徽章行」改为「计数 + 点击展开」——反馈 3 的「散开」问题即由此解决 |
...
| 节点 = 单组件卡（strImg 52px + 标题 + ≤2 行信息行） | 用户反馈 3：「每个节点应该做成单组件，图片+title…组件内布局紧凑展示」；参考 `BuildResponsesPanel` 的紧凑密度 |
| 分支卡只显示分支相关信息（物品/概率/前置条件数），不渲染目标 🎒🐾⚡ | 用户反馈 1：Hero 上下文徽章是「这个剧情有什么」；职责分清后同页不再重复 |
| 前置条件从「常显徽章行」改为「📋×n 计数 + 点击展开」 | 用户反馈 3：「条件使用目标等散开来…很散乱」；计数徽章收进卡片、细节按需展开，过滤交互本身保留（用户未抱怨） |
...
2. **节点单组件**：每条响应对应一张分支卡——卡内含 1 个 `Image`（或兜底 `SymbolIcon`）、
   标题 TextBlock（`Subject`）、`ID: {id}` 文本、类型标签；默认（未展开）状态下
   卡内前置条件仅以「📋×n」单个计数徽章出现，无散列的多个条件徽章。
...
6. **Mermaid 对齐**：`BuildMermaidText` 输出含当前与分支节点名称与 `(#id)`；每条边标签含
   权重与有效概率（`{w:F1}({p:P2})`）；有物品的边含物品名与 `×n`；有前置条件的目标边含
   `📋×n`；不出现反向 R 节点与 ctx 标签。
7. **过滤交互**：勾选某前置条件后，不满足的分支 `EffectiveProb == 0`、卡片 `Opacity == 0.5`、
   概率胶囊文本含 `0%`；满足分支的概率 = `Weight / 新有效总和`（纯函数断言具体数值）；
   Mermaid 文本同步更新（边标签出现 `⚠` 或 `0%`）。
8. **ResponsesPanel 合并**：`BuildDetail` 顶层不再出现独立「Vis.Responses」区块
   （`FindAll` 中节标题数量 = 0）；分支图节内存在格式提示行文本
   （`[物品ID]x[数量]=[剧情ID]x[权重]`）。
9. **类型映射**：`nType` 0/1/2/3 分别渲染为 剧情/搜刮/战斗/破解 四色 chip；未知值灰色兜底不崩溃。
10. **导航**：Ctrl+Click 分支卡 → `StubNavigationRouter` 记录跳转到目标 Encounter；
    Ctrl+RMB → peek；resolved 卡挂 tooltip（hover 有 `BuildRefTooltip` 内容）。
11. **当前卡**：含「📍 当前剧情」标识；当前卡不触发任何导航记录。
12. **回归**：TabControl 仍含 3 个 Tab（剧情分支 / 剧情链 / Mermaid源码）；`Responses` 为空时
