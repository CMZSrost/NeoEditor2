...
> - **动态 Dock 构建**：`Documents.cs` 删除 13 个手写 Tool 子类（OverlayChain/ValueEditor/ImagePreview/ReferenceInspector/SearchResults/KeyValueEditor/PeekPanel/DataTable/ForwardIndex/ReverseIndex/ImageAssetManager/ImageOrchestration/AiChatTool）→ 新增 `PluginTool`（Id=插件类型名，Title=plugin.Title，Content=Context=CreateToolView，CanClose=false）；`DocumentWorkspaceViewModel.BuildToolDock()` 枚举 `IEnumerable<IToolPlugin>` 按 `DefaultDock`/`Order` 分组构建 Left/Right/Bottom 三组 Tool 集合（左=1/右=2/底=2），XAML 手写 `<Tool>` 元素全删，Dock 容器保留。
> - **DataViewer 拆 5 plugin**：`DataViewerPlugin`（单）→ `DataTablePlugin`(Bottom,10)/`ForwardIndexPlugin`(Bottom,11)/`ReverseIndexPlugin`(Bottom,12)/`SearchPlugin`(Bottom,13)/`PeekPlugin`(Right,10)。DataTable 工具 Content=直接构造的 `ModGameDataTabsView`（绑共享 `ModDataToolViewModel`）。新增 `IIndexTableFactory` 提供 Forward/Reverse 共享 singleton。**Conflicts/Validation 因 9B 已删除，不建**。
> - **⚠️ 2026-08-02 订正（真机验证）**：Dock.Avalonia 12.1.0 的 `ToolDock.ItemsSource` **不同步工具进布局 VisibleDockables**（故 XAML ToolDock 不绑 ItemsSource）；工具改由 `DocumentWorkspaceViewModel.SyncToolDockIntoLayout(DockControl)` 在 `DockControl.Loaded` 用 `DockFactory.AddDockable` 按 `ToolDock.Id` 注入，DataTable 置为 Bottom 激活 tab；`Tool.Content` 必须是可直接构建视图（纯 VM 会崩 `Unexpected content`）。详见 CLAUDE.md 2026-08-02 段。
