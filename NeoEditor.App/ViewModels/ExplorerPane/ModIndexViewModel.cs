...
public partial class ModIndexViewModel : ViewModelBase, IRecipient<LoadProfileMessage>,
    ...
    IRecipient<GameRootDirChangedMessage>
{
    ...
    private readonly Core.Abstractions.IHostService _hostService;
    ...
    public ModIndexViewModel(IDbContextFactory<EditorDbContext> factory,
        ...
        Core.Abstractions.IHostService hostService)
        : base(localizationService, notificationService, logger)
    {
        ...
        // Quick DB scan
        Dispatcher.UIThread.InvokeAsync(async () => { await RefreshProfiles(); await RefreshDirtyCountAsync(); });
    }
    ...
    public async Task ImportProfile()
    {
        if (Application.Current?.ApplicationLifetime is IClassicDesktopStyleApplicationLifetime desktop)
        {
            ...
            {
                Title = Loc["SelectPhpProfile"], // 选择Php Mods文件
                AllowMultiple = true, // 允许多选
                FileTypeFilter = new[]
                {
                    new FilePickerFileType(Loc["PhpFiles"])
                    {
                        Patterns = new[] { "*.php" },
                    }
                }
            ...
            foreach (var folder in folders)
            {
                if (folder.TryGetLocalPath() is { } folderPath && !folderPath.Contains("getimages.php"))
                {
                    ...
                    {
                        Notification.ShowWarning($"Add {folderPath} failed: {e.Message}");
                    }
                }
            }
        }
    ...
    private async Task RefreshDirtyCountAsync()
    {
        ...
        {
            // Ensure ModLoadInfos is loaded (lazy)
        ...
        OnPropertyChanged(nameof(DirtyProfileCountText));
    }
    ...
    private async Task CompareProfiles(ProfileInfo? profileInfo)
    {
        ...
        {
            Notification.ShowInfo("No other profiles to compare with.", "Compare Profiles");
            return;
        }
    ...
    private void SetActiveProfile(ProfileInfo? profileInfo)
    {
        ...
        // R26 §3: scope HostService dirty tracking to this profile.
        _hostService.SetActiveProfile(profileInfo.ProfileId);
        ...
        string defaultExt, Func<string, Task> exportFunc, string? suggestedName = null)
    {
        ...
        try
        {
            ...
            Notification.ShowSuccess($"Exported to {savePath}", "Export");
        }
        catch (Exception ex)
        {
    ...
    public void Receive(LoadProfileMessage message)
    {
        ...
        {
            Notification.ShowWarning($"load {message.ProfileInfo.Path} failed: {e.Message}");
        }
    }
    ...
    public void Receive(SaveProfileMessage message)
    {
        try
        {
            ...
            Notification.ShowSuccess($"Profile {message.ProfileInfo.Name} saved successfully.");
            Dispatcher.UIThread.InvokeAsync(() => RefreshProfiles());
        }
        catch (Exception e)
        {
            Notification.ShowError($"Failed to save profile: {e.Message}");
        }
    }
}