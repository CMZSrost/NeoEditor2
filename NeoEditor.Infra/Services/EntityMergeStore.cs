...
public class EntityMergeStore
{
    ...
    // ── Reference index (SQLite-backed) ───────────────────────────────────

    /// <summary>In-memory SQLite reference index for the merge view.
    /// Created when the merge view opens, destroyed when it closes.</summary>
    public ReferenceIndexService? IndexService { get; set; }
    ...
    /// <summary>Open or create the in-memory index for this merge view.</summary>
    public void EnsureIndexService()
    {
        if (IndexService is not null) return;
        IndexService = ReferenceIndexService.CreateInMemory();
        IndexService.Open();
    }

    /// <summary>Close and dispose the in-memory index.</summary>
    public void DisposeIndexService()
    {
    ...
    public void Clear()
    {
        ...
        IndexService?.Clear();
        _index = null; // Reset lazy index so it rebuilds from fresh data
    }
}
