package org.flixel
{
   ...
   public class FlxCamera extends FlxBasic
   {
      ...
      public function set color(param1:uint) : void
      {
         ...
         _loc2_.greenMultiplier = (this._color >> 8 & 0xFF) * 0.00392;
         _loc2_.blueMultiplier = (this._color & 0xFF) * 0.00392;
      ...
      internal function drawFX() : void
      {
         ...
         if(this._fxFlashAlpha > 0)
         {
            ...
            this.fill((uint((_loc1_ <= 0 ? 255 : _loc1_) * this._fxFlashAlpha) << 24) + (this._fxFlashColor & 0xFFFFFF));
         }
         if(this._fxFadeAlpha > 0)
         {
            _loc1_ = this._fxFadeColor >> 24;
            this.fill((uint((_loc1_ <= 0 ? 255 : _loc1_) * this._fxFadeAlpha) << 24) + (this._fxFadeColor & 0xFFFFFF));
         }
      ...
      public function SetSize(param1:uint, param2:uint) : void
      {
         this.width = param1;
         this.height = param2;
         this.screen.pixels = new BitmapData(this.width,this.height,true,0);
         this.buffer = this.screen.pixels;
         var _loc3_:Boolean = this._flashBitmap.smoothing;
         this._flashBitmap.bitmapData = this.buffer;
         this._flashBitmap.x = -this.width * 0.5;
         this._flashBitmap.y = -this.height * 0.5;
         this._flashBitmap.smoothing = _loc3_;
         this._flashOffsetX = this.width * 0.5 * this.zoom;
         this._flashOffsetY = this.height * 0.5 * this.zoom;
         this._flashSprite.x = this.x + this._flashOffsetX;
         this._flashSprite.y = this.y + this._flashOffsetY;
